...
const cpuApi = require('../api/cpu');
const searchApi = require('../api/search');
const comparisonApi = require('../api/comparison');
const catalogApi = require('../api/ai-cpus');
...
// dados.js deve ser somente uma projeção gerada do JSON canônico.
const compatCode = fs.readFileSync(path.join(ROOT, 'dados.js'), 'utf8');
assert.ok(compatCode.includes('NÃO EDITE ESTE ARQUIVO'), 'dados.js não está marcado como artefato gerado');
const context = {};
...
// API individual.
let res = call(cpuApi, { cpu: ryzen.slug });
...
// Busca estruturada.
res = call(searchApi, { q: 'Ryzen 5 5600', limit: '5' });
...
res = call(catalogApi, { entity_id: ryzen.entity_id, limit: '1' });
...
// Comparação estruturada, quando o par editorial de referência existe.
const intel = dataset.processors.find(cpu => /core i5-12400f$/i.test(cpu.nome));
if (intel) {
  res = call(comparisonApi, { comparison: `${ryzen.slug}-vs-${intel.slug}` });
...
assert.ok(routes.includes('/api/cpu/:cpu'), 'rewrite da API individual ausente');
assert.ok(routes.includes('/api/comparison/:comparison*'), 'rewrite da API de comparação ausente');

console.log(`Camada de conhecimento validada: ${dataset.processors.length} CPUs, ${dataset.dataset_version}.`);
