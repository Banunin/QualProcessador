...
function carregarDatasetCpus() {
  ...
  if (fs.existsSync(DADOS_JSON_PATH)) {
    ...
    cacheDatasetCpus = {
      schema_version: Array.isArray(payload) ? null : payload.schema_version || null,
      benchmark_semantics: Array.isArray(payload) ? null : payload.benchmark_semantics || null,
      processors
    };
...
function cpuPublica(cpu) {
  return {
    ...cpu,
    canonical_url: BASE_URL + urlCpu(cpu),
    benchmarks: {
