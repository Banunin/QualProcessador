(function () {
    ...
    function init() {
        ...
        function adicionarAlias(alias, cpu) {
            ...
            if (!normal || normal.length < 5 || !/\d/.test(normal)) return;
            if (/^\d+$/.test(normal)) return;
            if (mapa.has(normal)) return;
            mapa.set(normal, cpu);
        ...
        listaDeCpus.forEach(cpu => {
            QPSeo.aliasesCpu(cpu).forEach(alias => adicionarAlias(alias, cpu));
        });
        ...
        let regex;
        try {
            regex = new RegExp(`\\b(${padroes.map(p => p.pattern).join('|')})\\b`, 'gi');
        } catch (e) {
            return;
        }
        ...
        function css() {
            ...
            style.textContent = `
                .qp-cpus-citadas{margin:34px 0 10px;padding:22px;border:1px solid #e2e8f0;border-radius:14px;background:#f8fafc;font-family:'Poppins',sans-serif}
                .qp-cpus-citadas h2{margin:0 0 7px!important;font-size:1.35rem!important}
                .qp-cpus-citadas>p{margin:0 0 16px!important;font-family:'Poppins',sans-serif;font-size:.9rem;color:#64748b}
                .qp-cpu-links-grid{display:grid;grid-template-columns:repeat(auto-fit,minmax(210px,1fr));gap:10px}
                .qp-cpu-link-card{display:flex;flex-direction:column;gap:4px;padding:13px 14px;border:1px solid #e2e8f0;border-radius:10px;background:white;text-decoration:none!important;color:#0f172a!important}
                .qp-cpu-link-card:hover{border-color:#38bdf8;transform:translateY(-1px)}
                .qp-cpu-link-card strong{font-size:.9rem}.qp-cpu-link-card span{font-size:.75rem;color:#64748b}
            `;
        ...
        function processar(body) {
            ...
            intro.textContent = 'Abra a ficha técnica dos modelos mencionados e continue a pesquisa no banco do QualProcessador.';
        ...
        function scan() {
            host.querySelectorAll('.article-body').forEach(processar);
        }
